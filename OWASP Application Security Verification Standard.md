...
- [Applying ASVS in Practice](#applying-asvs-in-practice)
- [How to Reference ASVS Requirements](#how-to-reference-asvs-requirements)
...
![OWASP Application Security Verification Standard 4.0 Levels](https://github.com/OWASP/ASVS/raw/master/4.0/images/asvs_40_levels.png)
...
The certifying organization must include in any report the scope of the verification (particularly if a key component is out of scope, such as SSO authentication), a summary of verification findings, including passed and failed tests, with clear indications of how to resolve the failed tests.
...
When the ASVS was first released, username + password was the most common form of authentication outside of high security systems. Multi-factor Authentication (MFA) was commonly accepted in security circles but rarely required elsewhere. As the number of password breaches increased, the idea that usernames are somehow confidential and passwords unknown, rendered many security controls untenable. For example, NIST SP 800-63 considers usernames and knowledge based authentication (KBA) as public information and passwords as secrets that have been compromised. This reality renders knowledge based authenticators, password history, complexity, and rotation controls largely ineffective. These controls always have been less than helpful, often forcing users to come up with weak passwords every few months, but with the release of over 5 billion username and password breaches, it's time to move on.

Of all the chapters in the ASVS, the authentication and session management chapters have changed the most. For this chapter to assist, we have provided a simple mapping to NIST's terminology, and we have rephrased many requirements to satisfy the intent of the requirement, rather than the letter of the requirement. For example, the ASVS uses the term "password" when NIST uses "memorized secret" throughout this standard.
...
Out of band verifiers are physical devices that can communicate with the verifier over a secure secondary channel. Examples include push notifications to mobile devices. This type of verifier should not be confused with SMS or voice, which are not secure channels. When a user wants to authenticate, the verifying application sends a message through a secure side-channel to a physical device such as a mobile phone, which the user then uses to confirm their authentication attempt.
...
Single-factor One-time Passwords (OTP) are physical or soft tokens that display a continually changing pseudo-random one-time challenge. These devices make phishing (impersonation) difficult, but not impossible. This type of authenticator is considered "something you have". Multi-factor authenticators, such as push notifications to mobile devices, or FIDO hardware authenticators, are considered more secure than single factor OTP.
...
Cryptographic security keys are smart cards or FIDO keys, where the user has a private key that unlocks when the card is put into a reader or FIDO client. They are considered "something you have". One popular implementation is Universal Second Factor (U2F) and FIDO. When using cryptographic keys, the user unlocks their device with a "something you know", such as a PIN or pattern, or a biometric scan "something you are", and certificates are used alongside, to identify the device and the verifier.
...
* Integrate with an existing and trusted federated identity provider that undertook the proper identification proofing process at the time of enrollment (such as government or FIDO) if required by risk profile
* Use an identity provider that uses NIST 800-63 rev 3 level 2 or greater 
* Directly undertake identity proofing using NIST 800-63 rev 3 level 2 or greater
...
* [OWASP Cheat Sheet: iOS - Security Decisions via Untrusted Inputs](https://cheatsheetseries.owasp.org/cheatsheets/iOS_Security_Cheat_Sheet.html#security-decisions-via-untrusted-inputs)
...
* [OWASP Cheat Sheet: AJAX Security](https://cheatsheetseries.owasp.org/cheatsheets/AJAX_Security_Cheat_Sheet.html)
* [OWASP Proactive Controls: Validate Inputs](https://owasp.org/www-project-proactive-controls/v3/en/c5-validate-inputs)
...
* [OWASP – Testing for SSL-TLS](https://owasp.org/www-project-web-security-testing-guide/stable/4-Web_Application_Security_Testing/09-Testing_for_Weak_Cryptography/01-Testing_for_Weak_SSL_TLS_Ciphers_Insufficient_Transport_Layer_Protection.html)
...
* [OWASP Testing Guide - Testing for Subdomain Takeover](https://owasp.org/www-project-web-security-testing-guide/stable/4-Web_Application_Security_Testing/02-Configuration_and_Deployment_Management_Testing/10-Test_for_Subdomain_Takeover.html)
...
* [OWASP AppSensor Detection Points](https://github.com/jtmelton/appsensor/blob/master/analysis/appsensor-detection-points.txt)
...
* [OWASP Protect FileUpload Against Malicious File](https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html#protect-against-malicious-files)
...
Note: Due to issues with XXE attacks against DTDs, DTD validation should be disabled and replaced with, for example, XSD validation.
...
* [JSON Web Tokens (JWT)](https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_Cheat_Sheet.html)
...
* [Exploiting CORS misconfiguration for BitCoins and Bounties](https://portswigger.net/research/exploiting-cors-misconfigurations-for-bitcoins-and-bounties)
* [OWASP Testing Guide 4.0: Configuration Management](https://owasp.org/www-project-web-security-testing-guide/v41/4-Web_Application_Security_Testing/02-Configuration_and_Deployment_Management_Testing/README.html)
...
* **CWE** – Common Weakness Enumeration, a community-developed list of common software security weaknesses. It serves as a common language, a measuring stick for software security tools, and as a baseline for weakness identification, mitigation, and prevention efforts.
* **DAST** – Dynamic application security testing, e.g. using a tool like OWASP ZAP
...
* **REST** – Representational State Transfer, a stateless architecture commonly used for web services
* **SAST** – Static application security testing
* **SBOM** – Software Bill of Materials, a list of components used by a software application
...
* [OWASP Internet of Things Project](https://owasp.org/www-project-internet-of-things/)
* [OWASP Serverless Project](https://owasp.org/www-project-serverless-top-10/)
* [OWASP DevSecOps Project](https://owasp.org/www-project-devsecops-guideline/)
...
* [MITRE Attack](https://attack.mitre.org/wiki/Main_Page)
* [SANS CWE 25](https://www.sans.org/top25-software-errors/)
* [NIST 800-63 - Digital Identity Guidelines](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-63-3.pdf)
...
### Control Objective
The Internet of Things (IoT) has become extremely popular, from consumer level devices such as home automation, routers, WiFi cameras, and fitness trackers, to Industrial Control Systems, smart city infrastructure, logistics, and telemetry for vehicles. IoT devices range from simple RFID tags to complex health care equipment. There is currently no single security standard for IoT manufacturers to follow, but that doesn't mean we have to accept insecure devices.

The requirements were developed in conjunction with the OWASP Internet of Things project leaders and represent the most common and highly probable security issues, with references to the OWASP IoT Top 10 2018 document.

### Security Verification Requirements

The requirements were developed in conjunction with the OWASP Internet of Things project leaders and represent the most common and highly probable security issues, with references to the OWASP Internet of Things Top 10 2018 document. Please refer to the current edition of the OWASP IoT Top 10 for detailed information on each requirement.