...
Injection flaws occur when an application sends untrusted data to an interpreter. Injection flaws are very prevalent, particularly in legacy code, often found in SQL queries, [[_content/dictionary#L|LDAP]] queries, [[_content/dictionary#X|XPath]] queries, [[_content/dictionary#O|OS]] commands, program arguments, etc. Injection flaws are easy to discover when examining code, but more difficult via testing. Scanners and fuzzers can help attackers find them.
...
An already productive application, which can be easily adapted. A Model-View-Controller ([[_content/dictionary#M|MVC]]) type application is just one example of having a easily accessible application architecture.
...
But also LDAP, [[_content/dictionary#S|SOAP]], XPath and [[_content/dictionary#R|REST]] based queries can be susceptible to injection attacks allowing for data retrieval or control bypass.
...
A successful SQL injection attack can read sensitive data from the database, modify database data (insert/update/delete), execute administration operations on the database (such as shutdown the [[_content/dictionary#D|DBMS]]), recover the content of a given file existing on the DBMS file system or write files into the file system, and, in some cases, issue commands to the operating system. SQL injection attacks are a type of injection attack, in which SQL commands are injected into data-plane input in order to affect the execution of predefined SQL commands.
SQL Injection attacks can be divided into the following three classes:

- Inband: data is extracted using the same channel that is used to inject the [[_content/dictionary#S|SQL]] code. This is the most straightforward kind of attack, in which the retrieved data is presented directly in the application web page.
- Out-of-band: data is retrieved using a different channel (e.g., an email with the results of the query is generated and sent to the tester).
- Inferential or Blind: there is no actual transfer of data, but the tester is able to reconstruct the information by sending particular requests and observing the resulting behavior of the [[_content/dictionary#D|DB]] Server.
...
Auditors should always look for uses of sp_execute, execute or exec within [[_content/dictionary#S|SQL]] Server stored procedures. Similar audit guidelines are necessary for similar functions for other vendors.
...
The time delay exploitation technique is very useful when the tester find a Blind SQL Injection situation, in which nothing is known on the outcome of an operation. This technique consists in sending an injected query and in case the conditional is true, the tester can monitor the time taken to for the server to respond. If there is a delay, the tester can assume the result of the conditional query is true. This exploitation technique can be different from [[_content/dictionary#D|DBMS]] to DBMS (check DBMS specific section).
http://www.example.com/product.php?id=10 [[_content/dictionary#A|AND]] [[_content/dictionary#I|IF]](version() like '5%', sleep(10), 'false'))--

In this example the tester is checking whether the [[_content/dictionary#M|MySql]] version is 5.x or not, making the server delay the answer by 10 seconds. The tester can increase the delay time and monitor the responses. The tester also doesn't need to wait for the response. Sometimes they can set a very high value (e.g. 100) and cancel the request after some seconds.
###### Out of band Exploitation technique¶
This technique is very useful when the tester find a Blind [[_content/dictionary#S|SQL]] Injection situation, in which nothing is known on the outcome of an operation. The technique consists of the use of [[_content/dictionary#D|DBMS]] functions to perform an out of band connection and deliver the results of the injected query as part of the request to the tester's server. Like the error based techniques, each DBMS has its own functions. Check for specific DBMS section.
###### ###### ##### Remediation¶
...
Various parts of SQL queries aren't legal locations for the use of bind variables, such as the names of tables or columns, and the sort order indicator ([[_content/dictionary#A|ASC]] or [[_content/dictionary#D|DESC]]). In such situations, input validation or query redesign is the most appropriate defense. For the names of tables or columns, ideally those values come from the code, and not from user parameters.
...
We have shown examples in Java, but practically all other languages, including Cold Fusion, and Classic [[_content/dictionary#A|ASP]], support parameterized query interfaces.
###### Safe Java Stored Procedure Example¶
The following code example uses a [[_content/dictionary#C|CallableStatement]], Java's implementation of the stored procedure interface, to execute the same database query. The sp_getAccountBalance stored procedure would have to be predefined in the database and implement the same functionality as the query defined above.
...
LDAP Injection is an attack used to exploit web based applications that construct LDAP statements based on user input. When an application fails to properly sanitize user input, it's possible to modify LDAP statements through techniques similar to [[_content/dictionary#S|SQL]] Injection. [[LDAP injection](https://owasp.org/www-community/attacks/LDAP_Injection)](https://owasp.org/www-community/attacks/LDAP_Injection) attacks could result in the granting of permissions to unauthorized queries, and content modification inside the LDAP tree. For more information on LDAP Injection attacks, visit LDAP injection.
...
Automated Exploitation¶
Scanner module of tool like [[_content/dictionary#O|OWASP]] [[[_content/dictionary#Z|ZAP]]](https://www.zaproxy.org/) have module to detect LDAP injection issue.
Remediation¶
###### Escape all variables using the right [[_content/dictionary#L|LDAP]] encoding function¶
The main way LDAP stores names is based on [[_content/dictionary#D|DN]] ([distinguished name](https://ldapwiki.com/wiki/Distinguished%20Names)). You can think of this like a unique identifier. These are sometimes used to access resources, like a username.
...
There are certain characters that are considered special characters in a [[_content/dictionary#D|DN]]. The exhaustive list is the following: \ # + < > , ; " = and leading or trailing spaces
Each DN points to exactly 1 entry, which can be thought of sort of like a row in a [[_content/dictionary#R|RDBMS]]. For each entry, there will be 1 or more attributes which are analogous to RDBMS columns. If you are interested in searching through [[_content/dictionary#L|LDAP]] for users will certain attributes, you may do so with search filters. In a search filter, you can use standard boolean logic to get a list of users matching an arbitrary constraint. Search filters are written in Polish notation [[_content/dictionary#A|AKA]] prefix notation.
Example:
...
Example code - Java¶
###### Safe Java for [[_content/dictionary#L|LDAP]] escaping Example¶
public String escapeDN (String name) {
 //From [[_content/dictionary#R|RFC]] 2253 and the / character for [[_content/dictionary#J|JNDI]]
...
public String escapeSearchFilter (String filter) {
 //From [[_content/dictionary#R|RFC]] 2254
...
##### [[_content/dictionary#X|XPath]] Injection¶
[[_content/dictionary#T|TODO]]
#### Scripting languages¶
...
[[_content/dictionary#O|OS]] command injection is a technique used via a web interface in order to execute OS commands on a web server. The user supplies operating system commands through a web interface in order to execute OS commands.
...
Out side of that, appending a semicolon to the end of a [[_content/dictionary#U|URL]] query parameter followed by an operating system command, will execute the command. %3B is URL encoded and decodes to semicolon. This is because the ; is interpreted as a command separator.
...
#### Network Protocols¶
Web applications often communicate with network daemons (like [[_content/dictionary#S|SMTP]], [[_content/dictionary#I|IMAP]], [[_content/dictionary#F|FTP]]) where user input becomes part of the communication stream. Here it is possible to inject command sequences to abuse an established session.
...
#### Rule #2 (Use a safe [[_content/dictionary#A|API]])¶
...
[[_content/dictionary#S|SQL]] Injection Prevention Cheat Sheet
[[OS_Command_Injection_Defense_Cheat_Sheet|[[_content/dictionary#O|OS]] Command Injection Defense Cheat Sheet]]
[[LDAP_Injection_Prevention_Cheat_Sheet|[[_content/dictionary#L|LDAP]] Injection Prevention Cheat Sheet]]
[[Injection_Prevention_in_Java_Cheat_Sheet|Injection Prevention Cheat Sheet in Java]]